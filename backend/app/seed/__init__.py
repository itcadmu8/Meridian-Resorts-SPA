    ...
    with SessionLocal() as session:
        ...
        try:
            ...
            if mongo_coll is not None:
                prefs_data = [
                    ("G-1001", ["vegetarian"], "Ocean view high floor", "Deep Tissue", "Late checkout requested"),
                    ("G-1002", ["halal"], "Quiet room away from elevator", "Aromatherapy", "Extra feather pillows"),
                    ("G-1003", ["gluten-free"], "Near spa facility", "Swedish Massage", "Morning room service"),
                    ("G-1004", ["vegan"], "Ground floor with patio", "Facial Treatment", "Non-feather pillows"),
                    ("G-1005", ["nut-allergy"], "High floor corner room", "Hot Stone", "Sparkling water welcome"),
                    ("G-1006", ["none"], "Standard quiet room", "Deep Tissue", "Extra towels"),
                    ("G-1007", ["dairy-free"], "Pool view suite", "Reflexology", "Evening turndown service"),
                    ("G-1008", ["none"], "High floor", "Hydrating Facial", "Soft pillows"),
                    ("G-1009", ["pescatarian"], "Oceanfront balcony", "Couples Massage", "Champagne on arrival"),
                    ("G-1010", ["none"], "Garden view room", "Swedish Massage", "Quiet zone preferred"),
                ]
        ...
        if session.query(models.Reservation).filter(models.Reservation.id.like("R-20%")).count() == 0:
        ...
        for staff_name in ["staff", "manager", "admin"]:
            if not session.query(models.User).filter_by(username=staff_name).first():
                session.add(models.User(username=staff_name, password_hash=default_staff_pw, role=models.UserRole.staff))
        ...
        if session.query(models.SpaAppointment).count() == 0:
            ...
            spa_seeds = [
                ("SPA-5001", "P-001", "Deep Tissue Massage", now + timedelta(hours=2), "Elena Rostova", "confirmed", "G-1001", "jamie.rivera@example.com", "Jamie Rivera"),
                ("SPA-5002", "P-001", "Hydrating Facial", now + timedelta(hours=4), "David Chen", "confirmed", "G-1002", "alex.mercer@example.com", "Alex Mercer"),
                ("SPA-5003", "P-002", "Aromatherapy Massage", now + timedelta(days=1, hours=2), "Maya Lin", "confirmed", "G-1003", "morgan.vance@example.com", "Morgan Vance"),
                ("SPA-5004", "P-002", "Hot Stone Therapy", now - timedelta(hours=3), "Sarah Jenkins", "completed", "G-1004", "taylor@example.com", "Taylor Smith"),
                ("SPA-5005", "P-003", "Swedish Body Massage", now + timedelta(hours=5), "Elena Rostova", "confirmed", "G-1005", "jordan@example.com", "Jordan Lee"),
                ("SPA-5006", "P-003", "Reflexology Treatment", now + timedelta(days=1), "David Chen", "confirmed", "G-1006", "casey@example.com", "Casey Martin"),
                ("SPA-5007", "P-004", "Deep Tissue Massage", now + timedelta(hours=1), "Maya Lin", "confirmed", "G-1007", "riley@example.com", "Riley Davis"),
                ("SPA-5008", "P-004", "Couples Wellness Ritual", now + timedelta(days=2), "Sarah Jenkins", "confirmed", "G-1008", "avery@example.com", "Avery Wilson"),
                ("SPA-5009", "P-005", "Radiance Facial & Scrub", now + timedelta(hours=3), "Elena Rostova", "confirmed", "G-1009", "quinn@example.com", "Quinn Taylor"),
                ("SPA-5010", "P-005", "Detox Hydrotherapy", now - timedelta(days=1), "David Chen", "completed", "G-1010", "dakota@example.com", "Dakota Evans"),
                ("SPA-5011", "P-006", "Thai Herbal Compress", now + timedelta(hours=6), "Maya Lin", "confirmed", "G-1011", "skyler@example.com", "Skyler Thomas"),
                ("SPA-5012", "P-006", "Deep Tissue Massage", now - timedelta(hours=6), "Sarah Jenkins", "cancelled", "G-1012", "jesse@example.com", "Jesse White"),
            ]
            for appt_id, p_id, service, starts, therapist, status, g_id, g_email, g_name in spa_seeds:
                session.add(
        ...
        if len(telemetry_guests) < 200:
            for i in range(1, 201):
                g_id = f"G-9{i:04d}"
                if not session.query(models.Guest).filter_by(id=g_id).first():
                    session.add(models.Guest(id=g_id, name=f"Telemetry Guest {i}", email=f"telemetry.{i}@internal.local", loyalty_tier="standard"))
            session.commit()
            telemetry_guests = session.query(models.Guest).filter(models.Guest.id.like("G-9%")).all()

        # Migration cleanup: Reassign any operational telemetry reservations (R-5xxxx / R-6xxxx) from G-10xx to background G-9xxx guests
        bg_guest_ids = [g.id for g in telemetry_guests]
        dirty_telemetry_res = session.query(models.Reservation).filter(
        ...
        existing_telemetry_res = session.query(models.Reservation).filter(
        ...
        if existing_telemetry_res < 5000:
            ...
            print(f"Seeding August - November 2026 operational telemetry across all 6 resorts...")

            prop_targets = {
                "P-001": {"arrivals": 11, "spa": 16, "covers": 105, "stay_duration": 3, "pax_per_res": 3},
                "P-002": {"arrivals": 10, "spa": 14, "covers": 95, "stay_duration": 3, "pax_per_res": 3},
                "P-003": {"arrivals": 9,  "spa": 13, "covers": 88, "stay_duration": 3, "pax_per_res": 3},
                "P-004": {"arrivals": 9,  "spa": 12, "covers": 82, "stay_duration": 3, "pax_per_res": 3},
                "P-005": {"arrivals": 9,  "spa": 12, "covers": 78, "stay_duration": 3, "pax_per_res": 3},
                "P-006": {"arrivals": 9,  "spa": 11, "covers": 72, "stay_duration": 3, "pax_per_res": 3},
            }
            ...
            while current_dt <= target_end:
                ...
                variation = 1.0 + (0.22 * math.sin(day_offset * 0.45)) + (0.12 if weekday in (4, 5, 6) else -0.06)
                ...
                for pid, targets in prop_targets.items():
                    ...
                    for s_idx in range(num_spa):
                        ...
                        appt_hour = 9 + (s_idx % 9)
                        appt_time = datetime.combine(current_dt, time(appt_hour, 0), tzinfo=UTC).replace(tzinfo=None)
                    ...
                    for meal_hour, covers in meal_slots:
                        if covers < 1:
                            continue
                        ord_counter += 1
                        placed_dt = datetime.combine(current_dt, time(meal_hour, (day_offset * 7) % 59), tzinfo=UTC).replace(tzinfo=None)
                        new_orders.append(
                            models.Order(
                                id=f"O-{ord_counter}",
                                property_id=pid,
            ...
            if new_reservations:
                ...
                session.bulk_save_objects(new_orders)
                session.commit()
            print(f"[OK] Successfully seeded Sept-Nov 2026 daily operational records! "
