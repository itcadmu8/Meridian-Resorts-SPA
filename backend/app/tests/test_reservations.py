...
def test_create_reservation_guest_counts(client, db_session):
    property_, guest, rate_plan = make_property_guest_plan(db_session)
    payload = {
        "guest_id": guest.id,
        "property_id": property_.id,
    ...
    assert data["adults"] == 6
    assert data["children"] == 4
    assert data["total_members"] == 10


def test_create_reservation_exceeds_max_members_limit(client, db_session):
    property_, guest, rate_plan = make_property_guest_plan(db_session)
    payload = {
        "guest_id": guest.id,
        "property_id": property_.id,
        ...
        "check_out": str(date.today() + timedelta(days=3)),
        "adults": 50,
        "children": 35,  # 85 members > max 80
    }
    create_resp = client.post("/api/v1/reservations", json=payload)
    assert create_resp.status_code == 400
    assert "Maximum 80 members" in create_resp.json()["detail"]
